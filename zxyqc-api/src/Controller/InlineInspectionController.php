...
class InlineInspectionController extends AppBaseController implements TokenAuthenticatedController
{
    public function indexAction($limit)
    {
        ...
        if($limit==0) {
            $max= 500;
            $min=$limit*$max;
            $where = " WHERE  quality_assignment_tbl.inline_inspection_assign_to In ($arr2)";
            $data_limit = "limit $limit,$max";

        }
        else
        {
            $where = " WHERE  quality_assignment_tbl.inline_inspection_assign_to In ($arr2) AND quality_assignment_tbl.id >$limit";
            $data_limit = "";
        }
...
FROM
 ...
 $where
GROUP BY id order by id desc $data_limit";
