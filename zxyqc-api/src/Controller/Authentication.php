...
class Authentication extends AppBaseController
{
    public function getTokenAction(Request $request)
    {
       ...
       $user_id = $user->contact_id;
        ...
        if($designation==237)
           $qc_filter = "WHERE hr_tbl.`designation` IN (237) AND hr_tbl.dept= '$div_id' AND hr_tbl.contact_id ='$user_id'";
        else {
            $qc_filter = "WHERE hr_tbl.`designation` IN (237) AND hr_tbl.dept ='$div_id'";

        }
       if ($designation==642 || $designation == 2097)
            $qa_filter = "WHERE hr_tbl.`designation` IN (642,2097) AND hr_tbl.dept= '$div_id' AND hr_tbl.contact_id ='$user_id'";
        else
            $qa_filter = "WHERE hr_tbl.`designation` IN (642,2097) AND hr_tbl.dept= '$div_id'" ;


     if($designation==659 || $designation == 9268 || $designation == 9348)
            $qam_filter = "WHERE hr_tbl.`designation` IN (659,9268,9348) AND hr_tbl.dept= '$div_id' AND hr_tbl.contact_id ='$user_id'";
        else
            $qam_filter = "WHERE hr_tbl.`designation` IN (659,9268,9348) AND hr_tbl.dept= '$div_id'";
...
$qam_filter
  ...
  AND hr_tbl.id != 0";
        ...
        $desig = $this->getRepository()->getResults($qam_sql);
...
$qa_filter
  ...
  AND hr_tbl.status != 1611 
  AND hr_tbl.id != 0";
        $qa = $this->getRepository()->getResults($sql2);
$sql = "SELECT 
  CONCAT_WS(
    ' ',
    contact_info.`fname`,
